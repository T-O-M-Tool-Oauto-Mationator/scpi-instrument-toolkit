...
| `type` | required | see below | Measurement type. |
...
scope meas 2 RMS          # print channel 2 RMS voltage
```
...
|-----------|----------|--------|-------------|
| `1-4\|all` | required | `1`–`4`, `all` | Channel to measure. |
| `type` | required | see [measurement types](#available-measurement-types) | Measurement type. |
| `label` | required | string, no spaces | **Name for this entry in the log.** Appears in `log print` output and used as the dictionary key in `calc` expressions: `m["label"]`. Use underscores — e.g. `ch1_freq`. |
| `unit=` | optional | string | Unit shown in `log print` (e.g. `Hz`, `V`). **Display-only** — does not affect stored value or calculations. |

```
scope meas_store 1 FREQUENCY meas_freq unit=Hz    # store as 'meas_freq'
scope meas_store 1 PK2PK     meas_pk2pk unit=V    # store as 'meas_pk2pk'
scope meas_store 1 RMS       meas_rms unit=V      # store as 'meas_rms'
```
...
```
scope meas_delay 1 2              # delay from ch1 rising edge to ch2 rising edge
