...
```bash
ev2300 read_byte 0x08 0x00    # read SYS_STAT on BQ76920
ev2300 read_byte 0x08 0x04    # read SYS_CTRL1
ev2300 read_byte 0x08 0x05    # read SYS_CTRL2
ev2300 read_byte 0x08 0x50    # read ADCGAIN1
ev2300 read_byte 0x08 0x51    # read ADCOFFSET
```
...
```bash
ev2300 write_byte 0x08 0x00 0xFF    # clear all faults (SYS_STAT = 0xFF) on BQ76920
ev2300 write_byte 0x08 0x04 0x10    # enable ADC (SYS_CTRL1 = 0x10)
ev2300 write_byte 0x08 0x05 0x03    # enable CHG + DSG (SYS_CTRL2 = 0x03)
ev2300 write_byte 0x08 0x09 0xBE    # set OV trip threshold
ev2300 write_byte 0x08 0x0A 0x97    # set UV trip threshold
```
...
## Typical workflow — BQ76920 battery monitor

Initialize a BQ76920 AFE (I2C address `0x08`), read ADC calibration, and monitor cell voltage:

```bash
# 1. Clear faults and enable the ADC
ev2300 write_byte 0x08 0x00 0xFF    # SYS_STAT = 0xFF (clear all faults)
ev2300 write_byte 0x08 0x04 0x10    # SYS_CTRL1 = 0x10 (ADC_EN)
ev2300 write_byte 0x08 0x05 0x03    # SYS_CTRL2 = 0x03 (CHG_ON + DSG_ON)
ev2300 write_byte 0x08 0x0B 0x19    # CC_CFG = 0x19 (coulomb counter config)
sleep 1.0

# 2. Verify registers were written
ev2300 read_byte 0x08 0x00          # SYS_STAT — should be 0x00 (no faults)
ev2300 read_byte 0x08 0x04          # SYS_CTRL1 — should be 0x10
ev2300 read_byte 0x08 0x05          # SYS_CTRL2 — should be 0x03

# 3. Read ADC calibration
ev2300 read_byte 0x08 0x50          # ADCGAIN1
ev2300 read_byte 0x08 0x59          # ADCGAIN2
ev2300 read_byte 0x08 0x51          # ADCOFFSET

# 4. Set OV/UV trip thresholds
ev2300 write_byte 0x08 0x09 0xBE    # OV_TRIP → ~4.301 V/cell
ev2300 write_byte 0x08 0x0A 0x97    # UV_TRIP → ~2.505 V/cell

# 5. Monitor during a voltage sweep
ev2300 read_byte 0x08 0x00          # check SYS_STAT for OV/UV flags
```

## Typical workflow — BQ40z50 fuel gauge

Read standard SBS registers from a smart battery fuel gauge (I2C address `0x16`):

```bash
ev2300 read_word 0x16 0x09        # Voltage()
ev2300 read_word 0x16 0x0D        # RelativeStateOfCharge()
ev2300 read_block 0x16 0x20       # ManufacturerName()
ev2300 scan 0x16                  # discover all readable registers
