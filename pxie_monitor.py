...
def example(resource_name, options, voltage_max, current_max, points_per_output_function, source_delay):
    with nidcpower.Session(resource_name=resource_name, options=options) as session:
        ...
        session.create_advanced_sequence(
            sequence_name="my_sequence", property_names=properties_used, set_as_active_sequence=True
        )
        ...
        with session.initiate():
            ...
            measurement_group = [
                session.channels[name].fetch_multiple(total_points, timeout=timeout) for name in channels
            ]
        ...
        for i, measurements in enumerate(measurement_group):
            ...
            for measurement in measurements:
                print(
                    line_format.format(
                        channel_name, num, measurement.voltage, measurement.current, str(measurement.in_compliance)
                    )
                )
                num += 1
...
def _main(argsv):
    parser = argparse.ArgumentParser(
        description="Output ramping voltage to voltage max, then ramping current to current max.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "-n", "--resource-name", default="PXI1Slot2/0, PXI1Slot3/0-1", help="Resource names of NI SMUs."
    )
